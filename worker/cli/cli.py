...
@click.option('--repo-location', type=click.Path)
...
def update_version(repo_namespace, repo_name, release_type, repo_location, trigger_id, commit_hash, yaml_config):
    ...
    branch_name = f'kebechet-version-update-{prefix}{new_release}'
    repo.git.checkout('-b', branch_name)
    with working_dir(repo_location):
        update_changelog(repo=repo, old_ref=prev_release, new_ref=commit_hash, new_release=new_release, prefix=prefix)
    ...
    repo.index.commit(f'Version {prefix}{new_release} Release')
    repo.remote().push()

    g = Github(ACCESS_TOKEN)
    github_repo = g.get_repo(f"{repo_location}/{repo_name}")
    github_repo.create_pull(
        title=f'Version {prefix}{new_release}',
...
def update_changelog(repo: Repo, old_ref, new_ref, new_version, prefix=''):
    if old_ref is None:  # git initial commit
        old_ref = repo.git.rev_list("HEAD", max_parents=0)
    changelog = repo.git.log(f'{old_ref}..{new_ref}')
    with open('CHANGELOG.md', 'a+') as f:  # creates file if it doesn't exist without overwriting it
        ...
        f.write(
            f"\n## Release {prefix}{new_release} ({datetime.now().replace(microsecond=0).isoformat()})\n"
        )
        f.write("\n".join(changelog) + "\n")
        f.write("".join(lines))
